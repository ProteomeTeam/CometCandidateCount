                             ...
                             struct sDBEntry *dbe)
{
   ...
   if(!CheckDuplicateJH(iWhichQuery, iStartResidue, iEndResidue, iStartPos, iEndPos, iFoundVariableMod, dCalcPepMass, szProteinSeq, bDecoyPep, piVarModSites, dbe)) {
      StorePeptideJH(iWhichQuery, iStartResidue, iStartPos, iEndPos, iFoundVariableMod, szProteinSeq, dCalcPepMass, dXcorr, bDecoyPep, piVarModSites, dbe);
      if( !memcmp( dbe->strName.c_str(), "XXX_", strlen("XXX_") ) ) {
	 //char jhPeptide[MAX_PEPTIDE_LEN];
	 //memcpy(jhPeptide, szProteinSeq+iStartPos, iLenPeptide*sizeof(char));
	 //jhPeptide[iLenPeptide] = '\0';
	 //printf("%d %s D\n",pQuery->_spectrumInfoInternal.iNumberOfDecoy,jhPeptide);
         pQuery->_spectrumInfoInternal.iNumberOfDecoy += 1;
      }
      else {
	 //char jhPeptide[MAX_PEPTIDE_LEN];
	 //memcpy(jhPeptide, szProteinSeq+iStartPos, iLenPeptide*sizeof(char));
	 //printf("%d %s T\n",pQuery->_spectrumInfoInternal.iNumberOfTarget,jhPeptide);
	 //jhPeptide[iLenPeptide] = '\0';
         pQuery->_spectrumInfoInternal.iNumberOfTarget += 1;
      }
   }
...
int CometSearch::CheckDuplicateJH(int iWhichQuery,
                                int iStartResidue,
                                int iEndResidue,
                                int iStartPos,
                                int iEndPos,
                                ...
                                struct sDBEntry *dbe)
{
   ...
   //for (i=0; i<numJH; i++)
   for (i=0; i<pQuery->iMatchPeptideCountJH; i++)
   {
      // Quick check of peptide sequence length.
      if (iLenMinus1 == pQuery->_pResultsJH[i].iLenPeptide && isEqual(dCalcPepMass, pQuery->_pResultsJH[i].dPepMass))
      {
         if (!memcmp(pQuery->_pResultsJH[i].szPeptide, szProteinSeq + iStartPos, sizeof(char)*pQuery->_pResultsJH[i].iLenPeptide))
         {
            bIsDuplicate = 1;
         }
         else if (g_staticParams.options.bTreatSameIL)
         {
            ...
            for (int ii=iStartPos; ii<=iEndPos; ii++ )
            {
               if (pQuery->_pResultsJH[i].szPeptide[ii-iStartPos] != szProteinSeq[ii])
               {
                  if ((pQuery->_pResultsJH[i].szPeptide[ii-iStartPos]!='I' && pQuery->_pResultsJH[i].szPeptide[ii-iStartPos]!='L')
                        || (szProteinSeq[ii] != 'I' && szProteinSeq[ii] != 'L'))
                  {
                     bIsDuplicate = 0;
                     break;
         ...
         if (bIsDuplicate && g_staticParams.variableModParameters.bVarModSearch && iFoundVariableMod)
         {
            ...
            {
               int iVal;
               for (int ii=0; ii<=pQuery->_pResultsJH[i].iLenPeptide; ii++)
               {
                  iVal = pQuery->_pResultsJH[i].piVarModSites[ii];
                  ...
                  {
                     if (piVarModSites[ii] != pQuery->_pResultsJH[i].piVarModSites[ii])
                     {
                        bIsDuplicate = 0;
                        break;
                     }
                  ...
                  {
                     // must loop through each modsite and see if OBO string is same
                     if (strcmp(dbe->vectorPeffMod.at(-(piVarModSites[ii])-1).szMod, pQuery->_pResultsJH[i].pszMod[ii]))
                     {
                        bIsDuplicate = 0;
                        break;
                     }
            ...
            {
               if (!memcmp(piVarModSites, pQuery->_pResultsJH[i].piVarModSites, sizeof(int)*(pQuery->_pResultsJH[i].iLenPeptide + 2)))
                  bIsDuplicate = 1;
               else
                  bIsDuplicate = 0;
            }
         }
         if( bIsDuplicate ) break;
      }
   }
   return (bIsDuplicate);
}
void CometSearch::StorePeptideJH(int iWhichQuery,
                               int iStartResidue,
                               int iStartPos,
                               int iEndPos,
                               int iFoundVariableMod,
                               ...
                               struct sDBEntry *dbe)
{
   ...
   if (iLenPeptide >= MAX_PEPTIDE_LEN)
      return;
   int index = pQuery->iMatchPeptideCountJH++;

   pQuery->_pResultsJH[index].iLenPeptide = iLenPeptide;

   memcpy(pQuery->_pResultsJH[index].szPeptide, szProteinSeq+iStartPos, iLenPeptide*sizeof(char));
   pQuery->_pResultsJH[index].szPeptide[iLenPeptide]='\0';
   pQuery->_pResultsJH[index].dPepMass = dCalcPepMass;
 
   if (g_staticParams.variableModParameters.bVarModSearch)
   {
      if (!iFoundVariableMod)  // Normal peptide in variable mod search.
      {
         memset(pQuery->_pResultsJH[index].piVarModSites, 0, _iSizepiVarModSites);
         memset(pQuery->_pResultsJH[index].pdVarModSites, 0, _iSizepdVarModSites);
      }
      else
      {
         memcpy(pQuery->_pResultsJH[index].piVarModSites, piVarModSites, _iSizepiVarModSites);
 
         int iVal;
         for (i=0; i<iLenPeptide + 2; i++)
         {
            iVal = pQuery->_pResultsJH[index].piVarModSites[i];
 
            if (iVal > 0)
            {
               pQuery->_pResultsJH[index].pdVarModSites[i]
                  = g_staticParams.variableModParameters.varModList[iVal-1].dVarModMass;
            }
            else if (iVal < 0)
            {
               pQuery->_pResultsJH[index].pdVarModSites[i] = dbe->vectorPeffMod.at(-iVal-1).dMassDiffMono;
               strcpy(pQuery->_pResultsJH[index].pszMod[i], dbe->vectorPeffMod.at(-iVal-1).szMod);
            }
            else
               pQuery->_pResultsJH[index].pdVarModSites[i] = 0.0;
         }
      }
   }
   else
