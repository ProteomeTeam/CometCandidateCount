...
static bool AllocateResultsMem()
{
   for (std::vector<Query*>::iterator it = g_pvQuery.begin(); it != g_pvQuery.end(); ++it)
   {
      ...
      try
      {
         pQuery->_pResultsJH = new ResultsJH[numJH];
      }
      catch (std::bad_alloc& ba)
      {
         char szErrorMsg[256];
         sprintf(szErrorMsg, " Error - new(_pResultsJH[]). bad_alloc: %s.\n", ba.what());
         string strErrorMsg(szErrorMsg);
         g_cometStatus.SetStatus(CometResult_Failed, strErrorMsg);
         logerr(szErrorMsg);
         return false;
      ...
      for (int j=0; j<numJH; j++)
      {
         pQuery->_pResultsJH[j].dPepMass = 0.0;
         pQuery->_pResultsJH[j].iLenPeptide = 0;
         pQuery->_pResultsJH[j].szPeptide[0] = '\0';
      }
   }
