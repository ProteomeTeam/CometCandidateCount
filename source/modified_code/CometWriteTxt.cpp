                                 ...
                                 FILE *fpdb)
{
         ...
         || (bDecoy && g_pvQuery.at(iWhichQuery)->_pDecoys[0].fXcorr > XCORR_CUTOFF))
   {
      ...
      for (int iWhichResult=0; iWhichResult<iNumPrintLines; iWhichResult++)
      {
         ...
         fprintf(fpout, "%d\t", pQuery->_spectrumInfoInternal.iNumberOfTarget);
         fprintf(fpout, "%d\t", pQuery->_spectrumInfoInternal.iNumberOfDecoy);
         double calculation = pQuery->_spectrumInfoInternal.iNumberOfDecoy / (double)( pQuery->_spectrumInfoInternal.iNumberOfTarget + pQuery->_spectrumInfoInternal.iNumberOfDecoy );
         fprintf(fpout, "%0.6f\t", calculation);
